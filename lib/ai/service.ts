...
export async function processAIQuery(query: string): Promise<AIResponse> {
    ...
    if (GREETINGS.some(g => q.includes(g))) {
        return {
            message: '¡Hola! Soy tu asistente virtual de LocalEcomer. ¿En qué puedo ayudarte hoy?\nPuedo buscar productos, llevarte a tiendas o responder dudas.',
    ...
    if (HELP_KEYWORDS.some(k => q.includes(k))) {
        return {
            message: 'Estoy aquí para ayudarte a navegar el centro comercial digital. Puedes pedirme:\n- "Buscar zapatillas Nike"\n- "Ir a la tienda de tecnología"\n- "Ver ofertas flash"',
    ...
    const stopwords = ['busco', 'quiero', 'necesito', 'unos', 'unas', 'el', 'la', 'los', 'las', 'de', 'en', 'para']
