...
export function getMarketplaceProducts(
  category: MarketplaceCategory = 'Todos'
): MarketplaceProduct[] {
...
export function getFlashDeals(limit = 6): MarketplaceProduct[] {
  return [...marketplaceProducts].sort((a, b) => b.discount - a.discount).slice(0, limit)
}
...
export function getActiveStores(): {
  ...
  const storeMap = new Map<
    string,
    { name: string; url: string; color: string; productCount: number }
  >()
