...
import {
    Search, ShoppingBag, Heart, Plus, X, Minus, Trash2,
    Star, ChevronRight, User, ArrowLeft,
} from 'lucide-react'
...
export interface RealProduct {
    ...
    images: { full: string; thumbnail: string; isMain: boolean }[] | any
    stock: number
    is_active: boolean
}
...
export default function MinimalTemplate({ store, products }: MinimalTemplateProps) {
    ...
    const dynamicCats = Array.from(new Set(products.map(p => p.category_id || 'Otros').filter(Boolean)))
    ...
    const cartTotal = cart.reduce((sum, item) => sum + (item.product.discount_price || item.product.price) * item.quantity, 0)
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                alert(data.error || 'Ocurrió un error al procesar tu pedido. Comprueba que tengas sesión iniciada.')
            ...
            const orderText = `¡Hola! Acabo de registrar el pedido *#${orderId.slice(0, 8)}* en tu tienda virtual ${store.name}.\n\n` +
    ...
    return (
        <div className="mn-store" style={customStyle}>
            ...
            <header className="mn-header">
                ...
                <div className="mn-header-right">
                    ...
                    >
                        <ShoppingBag size={20} />
                        {cartCount > 0 && (
                            <span className="mn-cart-count">{cartCount}</span>
                        )}
                    </button>
                </div>
            </header>
            ...
            <section className="mn-hero">
                <div className="mn-hero-badge" style={{ background: store.theme_color || '#111', color: '#fff' }}>
                    ...
                    <div className="mn-categories">
                        {categories.map(cat => (
                            <button
                                ...
                                style={activeCategory === cat ? { background: store.theme_color || '#111', color: '#fff' } : {}}
                            >
                                {cat}
                            </button>
                        ))}
            ...
            <section className="mn-section" style={{ paddingTop: categories.length > 1 ? 0 : '2rem' }}>
                <div className="mn-section-header">
                    ...
                    <span className="mn-section-link">
                            ...
                            return (
                                <div key={product.id} className="mn-product-card">
                                    <div className="mn-product-image">
                                        ...
                                        />
                                        {isDiscounted && (
                                            <span className="mn-product-badge" style={{ background: store.theme_color || '#e94560' }}>OFERTA</span>
                                        )}
                                        <button
                                        ...
                                        >
                                            <Heart
                                                size={16}
                                                fill={wishlist.includes(product.id) ? (store.theme_color || '#e94560') : 'none'}
                                                stroke={wishlist.includes(product.id) ? (store.theme_color || '#e94560') : '#111'}
                                            />
                                        </button>
                                    </div>
                                    <div className="mn-product-info">
                                        ...
                                        <div className="mn-product-price-row">
                                            <span className="mn-product-price">${finalPrice.toLocaleString('es-CO')}</span>
                                            {isDiscounted && (
                                                <>
                                                    <span className="mn-product-original-price">
                                                        ${product.price.toLocaleString('es-CO')}
                                                    </span>
                                                    <span className="mn-product-discount" style={{ color: store.theme_color || '#e94560' }}>
            ...
            {/* ─────────── Featured Banner ─────────── */}
            {filteredProducts.length > 0 && (
                <section className="mn-featured-banner" style={{ background: `linear-gradient(135deg, ${store.theme_color || '#111'}dd, ${store.theme_color || '#111'})` }}>
                    <div className="mn-featured-label" style={{ color: store.theme_color || '#111' }}>✦ GARANTIZADO</div>
                    ...
                    <div className="mn-cart-sidebar">
                        ...
                        {cart.length === 0 ? (
                            <div className="mn-cart-empty">
                                <div className="mn-cart-empty-icon" style={{ color: store.theme_color || '#111', background: `${store.theme_color || '#111'}11` }}>
                                    <ShoppingBag size={32} />
                                </div>
                            ...
                            <>
                                    ...
                                    {cart.map(item => (
                                        <div key={item.product.id} className="mn-cart-item">
                                            ...
                                            <div className="mn-cart-item-info">
                                                ...
                                                <div className="mn-cart-item-price" style={{ color: store.theme_color || '#111' }}>
                                                    ${((item.product.discount_price || item.product.price) * item.quantity).toLocaleString('es-CO')}
                                                </div>
                                ...
                                <div className="mn-cart-footer">
                                    <div className="mn-cart-total">
                                        ...
                                        <span className="mn-cart-total-value" style={{ color: store.theme_color || '#111' }}>
                                    ...
                                    >
                                        {isCheckingOut ? 'Asegurando pedido...' : 'Registrar Pedido y Enviar a WhatsApp'}
                                    </button>
                                </div>
                            </>
                        )}
