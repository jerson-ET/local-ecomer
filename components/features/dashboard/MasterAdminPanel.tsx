...
import { Store, Crown, AlertTriangle, ChevronRight, Activity } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'

export default function MasterAdminPanel() {
  const [stats, setStats] = useState({ users: 0, stores: 0, products: 0 })
  const [recentStores, setRecentStores] = useState<
    { id: string; store_name: string; slug: string; created_at: string; category: string }[]
  >([])
  ...
  return (
    <div className="admin-panel">
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #1e1e2f 0%, #2d2b4f 100%)', color: 'white' }}
      >
        <div className="admin-header-content">
          <h1>
            <Crown size={28} style={{ color: '#f39c12', display: 'inline', marginRight: '8px' }} />{' '}
            Panel Maestro
          </h1>
      ...
      <div className="admin-grid">
        <div className="admin-card">
          <div className="card-header">
            <h3>
              <Activity size={20} /> Auditoría en Tiempo Real
            </h3>
          </div>
          <div className="card-content">
            <p style={{ color: '#666', fontSize: '0.9rem', marginBottom: '1rem' }}>
              Vigilancia activa de creaciones de tiendas e infracciones de políticas.
            </p>
            <ul
              style={{
                listStyle: 'none',
              ...
              {recentStores.map((store) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <div>
                      <h4 style={{ margin: 0, fontSize: '0.9rem', color: '#111' }}>
                        {store.store_name}
                      </h4>
                      <span style={{ fontSize: '0.75rem', color: '#666' }}>
                        /{store.slug} - {store.category}
                      </span>
                    </div>
                  </div>
                  <button
        ...
        <div className="admin-card">
          <div className="card-header">
            <h3>
              <AlertTriangle size={20} color="#e74c3c" /> Alertas de la Plataforma
            </h3>
          </div>
          <div
          ...
          >
            ...
            <p style={{ fontSize: '0.85rem', color: '#999', maxWidth: '80%' }}>
              El sistema está operando normalmente. Las tiendas están dentro de los límites de
              políticas.
            </p>
          </div>
        </div>
