...
async function run() {
  ...
  for (const user of users) {
    ...
    let codeUsed = meta.referred_by || meta.referred_by_code || null;
    if (codeUsed) codeUsed = codeUsed.toUpperCase();
    
    const isPro = meta.paid_until && new Date(meta.paid_until).getTime() > Date.now() + 7 * 24 * 60 * 60 * 1000;
    
    if (codeUsed && isPro) {
       console.log(`\n\n✅ [PRO USER] ${meta.nombre || user.email} usó el código ${codeUsed}`);
       // Busquemos a su papá
       const parent = users.find(u => u.user_metadata?.referral_code === codeUsed);
       if (parent) {
         console.log(`   -> Su papá es ${parent.user_metadata?.nombre || parent.email}`);
         const pMeta = parent.user_metadata || {};
         let prospects = Array.isArray(pMeta.affiliate_prospects) ? pMeta.affiliate_prospects : [];
         
         const prospectMatch = prospects.find(p => p.id === user.id || p.whatsapp === meta.telefono || p.name === meta.nombre);
         if (prospectMatch) {
            ...
            if (prospectMatch.status === 'pending') {
               ...
               const updatedProspects = prospects.map(p => p.id === prospectMatch.id || p.whatsapp === meta.telefono ? { ...p, status: 'active', id: user.id } : p);
         ...
         console.log(`   ⚠️ Padre con código ${codeUsed} no existe!`);
       }
    }
  }
