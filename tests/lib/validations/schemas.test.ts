...
describe('PasswordSchema', () => {
    ...
    it('debe requerir al menos una mayúscula', () => {
        ...
        expect(result.success).toBe(false)
        if (!result.success) {
            const hasUppercaseError = result.error.issues.some(
                (issue) => issue.message.includes('mayúscula')
            )
    ...
    it('debe requerir al menos una minúscula', () => {
        ...
        expect(result.success).toBe(false)
        if (!result.success) {
            const hasLowercaseError = result.error.issues.some(
                (issue) => issue.message.includes('minúscula')
            )
    ...
    it('debe requerir al menos un número', () => {
        ...
        expect(result.success).toBe(false)
        if (!result.success) {
            const hasNumberError = result.error.issues.some(
                (issue) => issue.message.includes('número')
            )
