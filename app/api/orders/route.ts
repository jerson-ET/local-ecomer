...
export async function POST(request: Request) {
    try {
        ...
        if (!storeId || !items || !Array.isArray(items) || items.length === 0) {
            return NextResponse.json({ error: 'Faltan parámetros requeridos o el carrito está vacío' }, { status: 400 })
        }
        ...
        for (const item of items) {
            ...
            if (Array.isArray(prodData.images) && prodData.images.length > 0) {
                const mainImg = prodData.images.find((i: any) => i.isMain) || prodData.images[0]
