    ...
    try {
        ...
        const { data: newStore, error: insertError } = await supabase
            ...
            .insert({
                ...
                whatsapp_number: body.whatsappNumber?.replace(/\D/g, '') || null,
        ...
        /* 7. Actualizar el rol del usuario a 'seller' si era 'buyer' */
        await supabase
            .from('profiles')
            .update({ role: 'seller' })
            .eq('id', user.id)
            .eq('role', 'buyer') /* Solo actualizar si era buyer */
...
export async function GET() {
    try {
        ...
        const { data: stores, error } = await supabase
            .from('stores')
            .select('id, name, slug, theme_color, banner_url, is_active, plan, whatsapp_number, created_at')
            .eq('user_id', user.id)
