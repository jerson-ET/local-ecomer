...
import {
    ...
    Crown, Hexagon, ShoppingCart, User
} from 'lucide-react'
...
const CAPS: Cap[] = [
    {
        ...
        color: 'Black/Gold',
        price: 159900,
        image: 'https://images.unsplash.com/photo-1588850561407-ed78c282e89f?auto=format&fit=crop&w=600&q=80',
        category: 'Snapbacks'
    },
    {
        id: '2',
        ...
        color: 'Faded Blue',
        price: 89900,
        image: 'https://images.unsplash.com/photo-1575428652377-a2697240dac0?auto=format&fit=crop&w=600&q=80',
        category: 'Dad Hats'
    },
    {
        id: '3',
        ...
        <div className="cap-store">
            ...
            <div className="ck-header">
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button className="ck-icon-btn"><Search size={24} /></button>
                    <button className="ck-icon-btn" onClick={() => setCartOpen(true)}>
                        <ShoppingBag size={24} />
                        {cart.length > 0 && <span style={{ position: 'absolute', top: 15, right: 20, width: 8, height: 8, background: 'var(--ck-neon)', borderRadius: '50%' }} />}
                    </button>
                </div>
            </div>
            ...
            <div className="ck-hero">
                <img src="https://images.unsplash.com/photo-1588850561407-ed78c282e89f?auto=format&fit=crop&w=400&q=80" alt="New Era Cap" className="ck-hero-img" />
                <h1>CROWN <span>YOUR CITY</span></h1>
            ...
            <div className="ck-grid">
                {filtered.map(cap => (
                    <div key={cap.id} className="ck-prod">
                        ...
                        <button className="ck-add-btn" onClick={() => addToCart(cap)}>+</button>
                    </div>
                ))}
            </div>
            ...
            <div className="ck-lab">
                <div className="ck-lab-content">
                    <h2 style={{ color: 'var(--ck-white)' }}>CUSTOM <span style={{ color: 'var(--ck-neon)' }}>PATCH LAB</span></h2>
                    <p style={{ marginBottom: 16, fontSize: 12, color: 'var(--ck-white)' }}>Crea parches personalizados para tu gorra.</p>
            ...
            <div className="ck-nav">
                <button className="ck-nav-item active"><Crown size={24} /></button>
                <button className="ck-nav-item"><Search size={24} /></button>
                <button className="ck-nav-item" onClick={() => setCartOpen(true)}><ShoppingCart size={24} /></button>
                <button className="ck-nav-item"><User size={24} /></button>
            </div>
            ...
            {arOpen && (
                <div className="ck-cart-overlay" style={{ zIndex: 100 }} onClick={() => setArOpen(false)}>
                    <div
                        ...
                        style={{ width: '90%', maxWidth: '600px', height: 'auto', background: 'black', border: '1px solid #333' }}
                        onClick={(e) => e.stopPropagation()}
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                            <h2 style={{ fontFamily: 'Anton', textTransform: 'uppercase', color: 'var(--ck-neon)' }}>Probador Virtual</h2>
                            <button onClick={() => setArOpen(false)} style={{ background: 'none', border: 'none', color: 'white' }}><X /></button>
                        </div>
            ...
            {cartOpen && (
                <div className="ck-cart-overlay" onClick={() => setCartOpen(false)}>
                    <div className="ck-cart-box" onClick={(e) => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                            <h2 style={{ fontFamily: 'Anton', textTransform: 'uppercase', color: 'var(--ck-neon)' }}>Tu Stash</h2>
                        ...
                        {cart.length === 0 ? (
                            ...
                            <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 16, borderBottom: '1px solid #333', paddingBottom: 10 }}>
                                ...
                                <div style={{ flex: 1 }}>
                                    ...
                                    <div style={{ color: 'var(--ck-neon)', fontSize: 12 }}>{fmt(c.item.price * c.qty)}</div>
                                    <div style={{ fontSize: 10, color: '#888' }}>x{c.qty}</div>
                                </div>
                            </div>
                        ))}
                        {cart.length > 0 && (
                            <button style={{ width: '100%', padding: 16, background: 'var(--ck-neon)', border: 'none', fontWeight: 900, fontSize: 16, textTransform: 'uppercase', marginTop: 10 }}>
                                Checkout
                            </button>
                        )}
                    </div>
