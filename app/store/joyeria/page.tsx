        ...
        <div className="joyeria-store">
            <div className="jy-header">
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button className="jy-icon-btn"><Search size={24} /></button>
            ...
            <div className="jy-hero">
                ...
                <h1>Elegancia <span>Eterna</span></h1>
                <img
            ...
            <div className="jy-cats">
                {['Todos', 'Anillos', 'Collares', 'Pulseras', 'Aretes', 'Relojes'].map(cat => (
                    <div key={cat} className={`jy-cat ${filter === cat ? 'active' : ''}`} onClick={() => setFilter(cat)}>
                        {cat}
                    </div>
                ))}
            </div>
                    ...
                    <div key={product.id} className="jy-prod">
                        ...
                        <div className="jy-prod-info">
                            ...
                            <div className="jy-prod-bottom">
                                ...
                                <button className="jy-add-btn" onClick={() => addToCart(product)}>+</button>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="jy-nav">
                <button className="jy-nav-item active"><Gem size={24} /></button>
                <button className="jy-nav-item"><Search size={24} /></button>
                <button className="jy-nav-item" onClick={() => setCartOpen(true)}><ShoppingCart size={24} /></button>
                <button className="jy-nav-item"><User size={24} /></button>
            </div>
                ...
                <div className="jy-cart-overlay" onClick={() => setCartOpen(false)}>
                    <div className="jy-cart-box" onClick={(e) => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", color: 'var(--jy-gold)', fontWeight: 400, letterSpacing: 2 }}>Selección</h2>
                            <button onClick={() => setCartOpen(false)} style={{ background: 'none', border: 'none', color: 'var(--jy-champagne)', cursor: 'pointer' }}><X /></button>
                        </div>
                        {cart.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: 20, color: '#555', letterSpacing: 2, fontSize: 12 }}>SIN SELECCIÓN</div>
                        ) : cart.map((c, i) => (
                            <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 16, borderBottom: '1px solid rgba(212,175,55,0.2)', paddingBottom: 10 }}>
                                <img src={c.item.image} style={{ width: 50, height: 50, objectFit: 'cover' }} />
                                <div style={{ flex: 1 }}>
                                    <div style={{ fontWeight: 600, fontSize: 14 }}>{c.item.name}</div>
                                    <div style={{ color: 'var(--jy-gold)', fontSize: 12 }}>{fmt(c.item.price * c.qty)}</div>
                                    <div style={{ fontSize: 10, color: '#888' }}>x{c.qty}</div>
                                </div>
                            </div>
                        ))}
                        {cart.length > 0 && (
                            <button style={{ width: '100%', padding: 14, background: 'var(--jy-gold)', color: 'var(--jy-black)', border: 'none', fontWeight: 600, fontSize: 12, marginTop: 10, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: 3 }}>
                                Finalizar Compra
                            </button>
                        )}
                    </div>
