        ...
        <div className="luxury-store">
            <div className="lx-header">
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button className="lx-icon-btn"><Search size={24} /></button>
            ...
            <div className="lx-cats">
                {['Todos', 'Accesorios', 'Fragancias', 'Calzado'].map(cat => (
                    <div key={cat} className={`lx-cat ${filter === cat ? 'active' : ''}`} onClick={() => setFilter(cat)}>
                        {cat}
                    </div>
                ))}
            </div>
                    ...
                    <div key={product.id} className="lx-prod">
                        ...
                        <div className="lx-prod-info">
                            ...
                            <div className="lx-prod-bottom">
                                ...
                                <button className="lx-add-btn" onClick={() => addToCart(product)}>+</button>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="lx-nav">
                <button className="lx-nav-item active"><Crown size={24} /></button>
                <button className="lx-nav-item"><Search size={24} /></button>
                <button className="lx-nav-item" onClick={() => setCartOpen(true)}><ShoppingCart size={24} /></button>
                <button className="lx-nav-item"><User size={24} /></button>
            </div>
                ...
                <div className="lx-cart-overlay" onClick={() => setCartOpen(false)}>
                    <div className="lx-cart-box" onClick={(e) => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
                            <h2 style={{ fontFamily: "'Cinzel', serif", color: 'var(--lx-gold)', fontWeight: 400, letterSpacing: 4 }}>Selección</h2>
                            <button onClick={() => setCartOpen(false)} style={{ background: 'none', border: 'none', color: 'var(--lx-silver)', cursor: 'pointer' }}><X /></button>
                        </div>
                        {cart.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: 20, color: '#555', letterSpacing: 3, fontSize: 11, textTransform: 'uppercase' }}>Selección vacía</div>
                        ) : cart.map((c, i) => (
                            <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 16, borderBottom: '1px solid rgba(212,175,55,0.15)', paddingBottom: 10 }}>
                                <img src={c.item.image} style={{ width: 50, height: 50, objectFit: 'cover' }} />
                                <div style={{ flex: 1 }}>
                                    <div style={{ fontWeight: 600, fontSize: 14, letterSpacing: 1 }}>{c.item.name}</div>
                                    <div style={{ color: 'var(--lx-gold)', fontSize: 12 }}>{fmt(c.item.price * c.qty)}</div>
                                    <div style={{ fontSize: 10, color: '#666' }}>x{c.qty}</div>
                                </div>
                            </div>
                        ))}
                        {cart.length > 0 && (
                            <button style={{ width: '100%', padding: 16, background: 'var(--lx-gold)', color: 'var(--lx-black)', border: 'none', fontWeight: 600, fontSize: 11, marginTop: 10, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: 4 }}>
                                Checkout
                            </button>
                        )}
                    </div>
