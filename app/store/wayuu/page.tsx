...
const ITEMS: WayuuItem[] = [
    {
        ...
        image: 'https://images.unsplash.com/photo-1590874102752-ce22975f0f5d?auto=format&fit=crop&w=600&q=80'
    },
    {
        id: '2',
        ...
        image: 'https://images.unsplash.com/photo-1544965838-54ef8406f868?auto=format&fit=crop&w=600&q=80'
    },
    {
        id: '3',
    ...
    },
    {
        ...
        category: 'Accesorios',
        artisan: 'Jose Uriana',
        image: 'https://images.unsplash.com/photo-1572307480813-5b0aa701b0f5?auto=format&fit=crop&w=600&q=80'
    },
    {
        id: '5',
    ...
    },
    {
        ...
        price: 25000,
        category: 'Accesorios',
        image: 'https://images.unsplash.com/photo-1611082163836-234251234978?auto=format&fit=crop&w=600&q=80'
    }
]
        ...
        <div className="wayuu-store">
            ...
            <header className="wy-header">
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button className="wy-menu-btn"><Search size={22} /></button>
                    <button className="wy-cart-btn" onClick={() => setCartOpen(true)}>
                        <ShoppingBag size={22} />
                        {cart.length > 0 && <span style={{ fontSize: 10, verticalAlign: 'top', color: 'var(--wy-terra)', fontWeight: 'bold' }}>{cart.length}</span>}
                    </button>
                </div>
            </header>
            ...
            <section className="wy-hero">
                ...
                <div className="wy-hero-img-container">
                    <img src="https://images.unsplash.com/photo-1596464716127-f9a0859b4b ce?auto=format&fit=crop&w=600&q=80" alt="Wayuu Artisan" className="wy-hero-img" />
                </div>
                <h1 className="wy-hero-title">El Espíritu del <em>Tejido</em></h1>
                <p>Piezas únicas que cuentan historias ancestrales de la cultura Wayuu. Cada hilo es una tradición.</p>
                <button
                    style={{
                        background: 'var(--wy-terra)', color: 'white', border: 'none',
                        padding: '14px 28px', borderRadius: 30, fontSize: 14,
            ...
            <div className="wy-cats">
                {[
                    ...
                    <div key={c.name} className={`wy-cat-item ${activeCat === c.name ? 'active' : ''}`} onClick={() => setActiveCat(c.name)}>
                ...
                {filtered.map(item => (
                    <div key={item.id} className="wy-product">
                        <div className="wy-prod-img-box">
                            <span style={{ position: 'absolute', top: 10, right: 10, color: '#ccc' }}><Heart size={16} /></span>
                        ...
                        <div className="wy-prod-info">
                            ...
                            {item.artisan && <div style={{ fontSize: 11, color: '#888', marginBottom: 4, fontStyle: 'italic' }}>Por: {item.artisan}</div>}
                            <div className="wy-prod-price">{fmt(item.price)}</div>
                            <button className="wy-add-btn" onClick={() => addToCart(item)}>Agregar</button>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <section className="wy-story">
                ...
                <p>
                    "Tejer es escribir pensamientos". Para la mujer Wayuu, cada mochila es una extensión de su alma y su creatividad. Compra con propósito y apoya a las comunidades de La Guajira.
                </p>
            ...
            {cartOpen && (
                <>
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 199 }} onClick={() => setCartOpen(false)} />
                    <div className="wy-cart-panel">
                        ...
                        <div className="wy-cart-content">
                            {cart.length === 0 ? (
                                <div style={{ textAlign: 'center', padding: 40, color: '#888' }}>Tu bolsa está vacía</div>
                            ) : cart.map((c, i) => (
                                <div key={i} className="wy-cart-item">
                                    <img src={c.item.image} style={{ width: 50, height: 50, objectFit: 'cover', borderRadius: 4 }} />
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontWeight: 600 }}>{c.item.name}</div>
                                        <div style={{ fontSize: 12, color: 'var(--wy-terra)' }}>{fmt(c.item.price * c.qty)}</div>
                                        <div style={{ fontSize: 11, color: '#888' }}>Cant: {c.qty}</div>
                                    </div>
                                    <button style={{ background: 'none', border: 'none', color: '#aaa' }}><X size={14} /></button>
                                </div>
                            ))}
                        </div>
                        <div style={{ padding: 20, borderTop: '1px solid #eee' }}>
                            ...
                            <button style={{
                                width: '100%', padding: 16, background: 'var(--wy-terra)',
                                color: 'white', border: 'none', textTransform: 'uppercase',
                                letterSpacing: 1, fontSize: 12, fontWeight: 700
                            }}>
                                Finalizar Compra
                            </button>
                        </div>
