...
import { ShoppingCart, Search, Menu, X, ChevronRight, Trophy, Zap, Shield } from 'lucide-react'
...
export default function MotoPage({ store, products }: { store?: any; products?: any[] }) {
  ...
  // Mapear productos reales al UI model de MotoRacer
  const realProducts: Product[] = (products || []).map((p) => ({
    id: p.id,
    name: p.name,
    price: p.price,
  ...
  const displayProducts = realProducts.length > 0 ? [...realProducts, ...PRODUCTS] : PRODUCTS

  const filtered =
    activeCat === 'all' ? displayProducts : displayProducts.filter((p) => p.category === activeCat)
  ...
  return (
    <div className="moto-store">
      ...
      <header className="mt-header">
        ...
        <Link href="/tiendas" className="no-underline">
          <div className="mt-logo">
            MOTO<span style={{ color: 'var(--mt-primary)', fontStyle: 'italic' }}>RACER</span>
          </div>
        </Link>
        <div className="mt-nav-icons">
          ...
          <button className="mt-icon-btn" onClick={() => setCartOpen(true)}>
            ...
            {cart.length > 0 && (
              <span className="mt-cart-badge">{cart.reduce((a, c) => a + c.qty, 0)}</span>
            )}
          </button>
        </div>
      </header>
      ...
      <section className="mt-hero">
        <img
          src="https://images.unsplash.com/photo-1558981806-ec527fa84c3d?w=1200&q=80"
          alt="Moto Background"
          className="mt-hero-bg"
        />
        <div className="mt-hero-content">
          <h1>
            {store?.name?.toUpperCase() || 'DOMINA EL'} <span>ASFALTO</span>
          </h1>
          <p>
            {store?.description?.toUpperCase() || 'EQUIPAMIENTO PROFESIONAL PARA PILOTOS REALES'}
          </p>
        ...
        {filtered.map((p) => (
          <div key={p.id} className="mt-card">
            ...
            <div className="mt-card-body">
              ...
              {p.specs && (
                <div style={{ fontSize: 10, color: 'var(--mt-text-muted)', marginBottom: 4 }}>
                  {p.specs}
                </div>
              )}
              <div className="mt-card-price">
                {fmtPrice(p.price)}
                <button className="mt-add-btn" onClick={() => addToCart(p)}>
                  +
                </button>
              </div>
            </div>
      ...
      {cartOpen && (
        <>
          <div
            style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 199 }}
          ...
          <div className="mt-cart-sidebar">
              ...
              {cart.map((item, idx) => (
                <div key={idx} className="mt-cart-item">
                  ...
                  <div className="mt-cart-info">
                    ...
                    <div style={{ color: 'var(--mt-primary)', fontWeight: 'bold' }}>
            ...
            <button className="mt-checkout-btn">PROCESAR PAGO</button>
          </div>
        </>
      )}
    </div>
